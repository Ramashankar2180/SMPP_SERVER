...
#define NO_OF_COLS 15

char *table[] =  	{	"SNo int primary key auto_increment,",
						"ESMEAccName varchar(40) ,"			,
						"ESMEAccMsgId varchar(40) ,"		,	
						"SMSCId varchar(40) ,"				,
						"SMSCMsgId varchar(68),"			,
						"SourceAddr varchar(40),"			,
						"DestinationAddr varchar(40),"		,
						"RecvTime datetime,"				,
						"SubmittedStat int ,"				,
						"SubmitTime datetime,"				,
						"DeliveredStat int ,"               ,
...
int main(int argc, char **argv)
{
	...
	while( (opt = getopt(argc,argv,"h::v::u:p:d:m:s:o:n:f::")) !=EOF)
	{
		switch(opt)
		{
			...
			case 'm':
			{
				host_name = optarg;
				valid_option = 1;
				printf("\n-m option : host name is :%s",host_name);
				break;	
			}
			case 'f':
			{
	...
	*/
	...
	if(ans=='Y'||ans=='y')
	{
       ...
       sprintf(temp1,"%d",number);
       // for leap year checking the month
       sscanf(temp2,"%d",&number1);

		if(number1==2)
		{
    		strftime(temp3,20,"%Y",ptr);
			//sscanf(temp3,"%d",&number3);
			number3 = atoi(temp3);
		...
		/*if(number<=9)
		 ...
		 strcat(table_name,temp2);
			...
			else
			{
				...
				strcat(query,status_table_name);
				strcat(query,"(");
				strcat(query,fields[4]);
				strcat(query,",");
				strcat(query,fields[2]);
				strcat(query,",");
				strcat(query,fields[6]);
				strcat(query,",");
				strcat(query,fields[1]);
				strcat(query,");");
