...
typedef struct
{
	...
	Counter *msg_received;/*counter for smsc msg id for that client - only for tx*/
...
typedef struct
{
	...
	Counter *msg_sent;/*Number of deliver sm sent to the ESME*/
	/*to be added for rx
...
typedef struct 
{
	...
	List *tx_esme;				/*List storing data for tx connected sessions*/
	List *rx_esme;				/*List storing data for rx conncted sessions*/
	/*Sessions Related data*/
	
   long max_submits_unconnect; /*max no of submits allowed when smsc are unconnected, to accomodate -1 as well*/
   long max_submits; /*max no of submits allowed when smsc are unconnected, to accomodate -1 as well*/
	 long  max_tps;
	 long  msg_count;
	 int dnd_check;
	 time_t esm_time;
...
int  check_sender_api(Octstr **source_addr,Octstr *src_addr);
