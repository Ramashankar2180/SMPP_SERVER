...
#include "include/smpp_client_http.h"
...
char* client_get_status_name(sig_atomic_t program_status)
{
	char *ptr;
...
static Octstr *httpd_client_version(List *cgivars, int status_type)
{
	...
	printf("\nPROGRAM STATUS :%s\n",client_get_status_name(smpp_client_status));
	
	return  	octstr_format(GW_NAME ":Client Process %s.	"
								"%sProgram Status: %s		"
								"%sBuild: %s. 				"
								"%sCompiler: %s.			"
								"%sSystem: %s. 				"
...
static Octstr *httpd_client_shutdown(List *cgivars, int status_type)
{
    Octstr *reply;
	...
	info(0,"SMPP-SMSC  Received ADMIN SHUTDOWM command");
	system_shutdown();
        gwthread_wakeup(MAIN_THREAD_ID);
    }
    return octstr_create("Bringing SMSC process down");
}
Octstr *httpd_client_status(List *cgivars, int status_type)
{
	return print_client_status(status_type);
}

Octstr *httpd_smsc_status(List *cgivars, int status_type)
{
	return smsc2_status(status_type);
}

static Octstr *httpd_smsc_stop(List *cgivars, int status_type)
{
    Octstr *reply;
    Octstr *smsc;
    if ((reply = httpd_check_authorization(cgivars, 0))!= NULL) return reply;
...
static Octstr *httpd_smsc_restart(List *cgivars, int status_type)
{
    Octstr *reply;
    Octstr *smsc;
    if ((reply = httpd_check_authorization(cgivars, 0))!= NULL) return reply;
...
static struct httpd_command {
	...
	{ "version", httpd_client_version},
	{ "smpp_smsc_shutdown", httpd_client_shutdown},	
	{ "smpp_smsc_status", httpd_client_status},
	{ "smsc_status", httpd_smsc_status},
	{ "smsc_stop", httpd_smsc_stop},	
	{ "smsc_restart", httpd_smsc_restart},	
...
static void httpadmin_run(void *arg)
{
    ...
    while(smpp_client_status != CLIENT_DEAD) 
	{
		if (smpp_client_status == CLIENT_SHUTDOWN)
		{
...
int client_httpadmin_start(Cfg *cfg)
{
    CfgGroup *grp;
    int ssl = 0; 
...
void client_httpadmin_stop(void)
{
